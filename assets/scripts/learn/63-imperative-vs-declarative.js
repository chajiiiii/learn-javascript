...
// 전역 변수
const fruitsBasket =
  "키위 사과 배 참외 키위 망고 파인애플 키위 수박 키위".split(" ");
...
() => {
  ...
  const techTerms = [
    ...
    },
    {
      term: "프론트엔드",
      description:
        "사용자가 직접 보는 웹사이트나 애플리케이션의 화면 부분을 개발하는 영역입니다.",
    },
    {
      term: "백엔드",
      description:
        "서버, 데이터베이스 등 사용자가 직접 보지 않는 부분을 개발하는 영역입니다.",
    },
    {
      term: "API",
      description:
        "애플리케이션이 서로 소통할 수 있도록 도와주는 인터페이스입니다.",
    },
    {
      term: "클라우드 컴퓨팅",
      description:
        "인터넷을 통해 서버, 저장소, 소프트웨어 등 컴퓨팅 자원을 제공하는 기술입니다.",
    },
  ];
  ...
  () => {
    const descListTemplate = `
      <dl class="tech-terms">
        ${techTerms
          .map(
            ({ term, description }) => `<dt>${term}</dt><dd>${description}</dd>`
          )
          .join("")}
      </dl>
    `;
  ...
  () => {
    ...
    // 평탄화(Flatting)란?
    // [[1, 2], [3, 4]] -> [1, 2, 3, 4]
    const flattedSubjectArray = nestedSubjecArray.reduce(
      (result, subjectArray) => {
    ...
    // { 국어: 2, 수학: 2, ... }
    const subjectCountObject = flattedSubjectArray.reduce(
  ...
  (() => {
    const subjectCountObject = Object.values(timeTable)
      .flat()
      .reduce((object, key) => {
