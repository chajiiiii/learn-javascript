...

{
  ...
  indicatorWrapper.addEventListener("click", (e) => {
    ...
    // 방법 1. element.closest(selector) -> element 반환
    // console.log(target.closest(".carousel__indicator"));
    // const indicatorButton = target.closest(".carousel__indicator");
    // if (indicatorButton) {
    //   console.log(indicatorButton);
    // }

    // 방법 2. element.matches(selector) -> boolean 반환
    // console.log(target.matches(".carousel__indicator"));
    if (target.matches(".carousel__indicator")) {
      let selectedIndex;

      for (let i = 0, l = indicators.length; i < l; i += 1) {
        if (indicators.item(i) === target) {
          selectedIndex = i;
          break;
        }
      }
